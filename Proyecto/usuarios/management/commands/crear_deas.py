...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dea_data in deas:
            sede_asignada = random.choice(sedes)
            for modelo in dea_data["modelos"]:
                ...
                for espacio_obligado in EspacioObligado.objects.filter(
                    sede=sede_asignada
                ):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Se generaron y guardaron {len(deas)} registros en la tabla DEA y se registraron en la tabla intermedia deas_registrados"
            )
        )